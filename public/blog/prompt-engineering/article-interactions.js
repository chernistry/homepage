/**
 * Article Interactions for Prompt Engineering Feature
...
(function() {
    ...
    function ensureRootScope() {
        try {
            // Scope class for CSS
            document.body.classList.add('prompt-engineering-page');
        } catch (_) {
            // no-op, fail safe
        }
    }
    ...
    function setupScrollToTop() {
        ...
        scrollButton.innerHTML = '<svg class="icon" width="24" height="24" role="img" aria-hidden="true"><use href="#icon-up"></use></svg><span class="sr-only">Back to top</span>';
    ...
    function handleShare() {
        ...
        const text = 'The Science of Prompt Engineering: From Art to Applied Mathematics';
    ...
    function handleBookmark(button) {
        ...
        if (isBookmarked) {
            ...
            localStorage.removeItem('bookmarked-prompt-engineering');
            showNotification('Bookmark removed');
        } else {
            icon.textContent = 'bookmark';
            localStorage.setItem('bookmarked-prompt-engineering', 'true');
    ...
    function setupTooltips() {
        ...
        const tooltipData = {
            'PE2': 'Prompt Engineering 2.0 - A systematic framework for iterative prompt optimization using meta-prompts',
            ...
            'Chain‑of‑Thought': 'Chain-of-Thought - A prompting technique that instructs the model to show its reasoning process step-by-step',
            'ReAct': 'Reasoning and Acting - A framework combining reasoning traces and task-specific actions in language models',
            'MCTS': 'Monte Carlo Tree Search - A heuristic search algorithm for decision processes, adapted for prompt optimization',
            'AutoML': 'Automated Machine Learning - Techniques for automating the machine learning pipeline, including model selection',
            ...
            'Self‑Consistency': 'A technique that samples multiple reasoning paths and selects the most consistent answer',
            'Hard-negative': 'Hard-negative Sampling - Using challenging negative examples that are difficult to distinguish from positives',
            'Hard‑negative': 'Hard-negative Sampling - Using challenging negative examples that are difficult to distinguish from positives',
            'TextGrad': 'A framework for automatic differentiation through text, enabling gradient-based optimization of prompts',
            'RiOT': 'Residual Optimization Tree - An advanced prompt optimization method using tree search with residual connections',
            'AutoPDL': 'Automated Prompt Design Language - A system for automatically designing entire prompt programs',
            'APE': 'Automatic Prompt Engineer - An early system for automatically generating and testing prompt candidates',
            'OPRO': 'Optimization by PROmpting - Using language models as optimizers for various tasks including prompt engineering',
            'PAL': 'Program-aided Language models - A technique where models generate and execute code to solve problems',
            'STaR': 'Self-Taught Reasoner - A method for improving reasoning by learning from self-generated explanations',
            'Self-RAG': 'Self-Reflective Retrieval-Augmented Generation - A framework that retrieves passages on-demand',
            'Self‑RAG': 'Self-Reflective Retrieval-Augmented Generation - A framework that retrieves passages on-demand',
            'Self-Refine': 'A method for iteratively improving outputs through self-critique and refinement',
            'Self‑Refine': 'A method for iteratively improving outputs through self-critique and refinement',
            'Tree of Thoughts': 'A generalization of Chain-of-Thought that explores multiple reasoning paths in a tree structure',
            'Graph of Thoughts': 'An extension of Tree of Thoughts that allows for more complex reasoning graph structures',
    ...
    function initializeBookmarkState() {
        if (localStorage.getItem('bookmarked-prompt-engineering')) {
